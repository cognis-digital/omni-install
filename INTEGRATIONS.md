...
**omni-install** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `omni-install` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
omni-install ... --format json | cognis-connect emit --to sigma   # Sigma rules
omni-install ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
omni-install ... --format json | cognis-connect emit --to elastic --url $URL --token $TOK   # Elastic _bulk
omni-install ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `omni-install` outputs:

```python
from cognis_connect import normalize, sigma
findings = [normalize(rec, source="omni-install") for rec in records]   # records = your JSON output
