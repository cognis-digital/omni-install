...
# Canonical online catalog. When no local MANIFEST.json is found, the wizard
# falls back to the cognis-arsenal master manifest (raw GitHub URL). Override
# with --manifest <path-or-url> or the COGNIS_MANIFEST_URL env var.
...
def _fetch_manifest_url(url: str) -> Path | None:
    """Download a manifest URL into a temp file; return its Path (or None)."""
    import tempfile
    import urllib.request

    try:
        req = urllib.request.Request(url, headers={"User-Agent": "cognis-setup/1.0"})
        with urllib.request.urlopen(req, timeout=15) as r:  # nosec - explicit, user-pointed
            data = r.read()
        # Validate it parses as JSON before we hand it on.
        json.loads(data.decode("utf-8"))
        fd, tmp = tempfile.mkstemp(prefix="cognis_manifest_", suffix=".json")
        with os.fdopen(fd, "wb") as fh:
            fh.write(data)
        return Path(tmp)
    except Exception as exc:
        print(S.yellow(f"  (could not fetch manifest {url}: {exc})"))
        return None


def discover_manifest(explicit: str | None) -> Path | None:
    """
    Find a MANIFEST.json. Resolution order:
      1. explicit arg (a local path OR an http(s):// URL)
      2. COGNIS_MANIFEST_URL env var (URL)
      3. a few sensible local locations
      4. the canonical cognis-arsenal manifest URL (DEFAULT_MANIFEST_URL)
    """
    if explicit:
        if _looks_like_url(explicit):
            return _fetch_manifest_url(explicit)
        p = Path(explicit).expanduser()
        return p if p.is_file() else None

    env_url = os.environ.get("COGNIS_MANIFEST_URL")
    if env_url:
        fetched = _fetch_manifest_url(env_url)
        if fetched:
            return fetched
    ...
    # Last resort: the canonical online catalog (needs internet). If the fetch
    # fails (offline), we return None and the wizard still offers fleet + help.
    return _fetch_manifest_url(DEFAULT_MANIFEST_URL)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--manifest",
                        help="Path OR http(s):// URL to MANIFEST.json "
                             "(auto-discovered locally, else fetched from cognis-arsenal).")
